...
def test_create_sesh():
    ...
    response = client.post("/seshs", params={"sesh_length": 1, "sesh_type": "programming", "sesh_day": "2026-06-08"},
...
def test_update_sesh():
    ...
    response = client.put(f"/seshs/{sesh_id}", json={"length": 2, "sesh_type": "programming", "day": "2026-06-09"},
                           headers={"Authorization": f"Bearer {token}"})
    print(response.text)
    assert response.status_code == 200
