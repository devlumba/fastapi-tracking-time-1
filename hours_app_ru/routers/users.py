...
@router.post("/token", summary="Создание токена. Используется на заднем плане")
async def login_for_access_token(session: SessionDep, form_data: Annotated[OAuth2PasswordRequestForm, Depends()]) -> Token:  # depends on the type/class itself, i could put OAuth...Form in the parentheses of Depends
    ...
    if not user:  # this probably could be more explicit and understandable
        raise HTTPException(status_code=401, detail="Некорректный юзернейм или пароль",
...
@router.get("/users/me", summary="Просмотр своего профиля")
async def read_users_me(token: Annotated[str, Depends(oauth2_scheme)],
                        current_user: Annotated[UserInDB, Depends(get_current_user)]) -> UserBase:
    return current_user


@router.post("/users/", summary="Создание аккаунта")
...
@router.get("/users/", summary="Просмотреть все аккаунты")
async def read_users_all(session: SessionDep) -> list[UserPublic]:
    users = session.exec(select(UserInDB)).all()
    return users


@router.get("/users/{username:str}", summary="Просмотреть конкретного пользователя")
async def read_specific_user(session: SessionDep, username: str) -> UserPublic:
    user = session.exec(select(UserInDB).where(UserInDB.username==username)).first()
    if not user:
        raise HTTPException(status_code=404, detail='404 -> Пользователь Не Найден')
    return user


@router.get("/users/me/seshs", summary="Посмотреть Свои Записи", dependencies=[Depends(oauth2_scheme)])
